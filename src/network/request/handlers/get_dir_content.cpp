...
bool GetDirContentRH::getPath(tcp::socket& socket) {
...
ResponseId GetDirContentRH::checkPath() const {
    if (!fs::exists(path)) {
...
void GetDirContentRH::getFileList() {
    const SFS& sfs{client->getSFS()};
...
void GetDirContentRH::pushFileInList(const fs::path& file) {
...
bool GetDirContentRH::sendBuffer(tcp::socket& socket) const {
    BFLEncoder encoder{};
